...
func (s *HttpServer) processProperty(req *StartReq, tenappDir string) (propertyJsonFile string, logFile string, err error) {
        ...
        slog.Info("[调试] 前端发送的 properties", "properties", req.Properties, "requestId", req.RequestId, logTag)
        for extensionName, props := range req.Properties {
                if extensionName != "" {
                        slog.Info("[调试] 处理扩展属性", "extension", extensionName, "props", props, "requestId", req.RequestId, logTag)
                        for prop, val := range props {
                                slog.Info("[调试] 处理属性", "extension", extensionName, "prop", prop, "val", val, "requestId", req.RequestId, logTag)
