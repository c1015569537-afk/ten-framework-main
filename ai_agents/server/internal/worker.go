...
func (w *Worker) start(req *StartReq) (err error) {
        ...
        // Set LD_LIBRARY_PATH environment variable for agora_rtc SDK
        libraryPath := fmt.Sprintf("%s/ten_packages/system/agora_rtc_sdk/lib:%s/ten_packages/system/ten_runtime_go/lib:%s/ten_packages/extension/agora_rtc/lib", w.TenappDir, w.TenappDir, w.TenappDir)
        cmd.Env = append(os.Environ(), fmt.Sprintf("LD_LIBRARY_PATH=%s", libraryPath))
        slog.Info("Worker set LD_LIBRARY_PATH", "requestId", req.RequestId, "path", libraryPath, logTag)
