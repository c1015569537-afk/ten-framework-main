...
build_cxx_extensions() {
  ...
  for extension in $out/ten_packages/extension/speechmatics_asr_python; do
...
install_node_dependencies() {
  ...
  if [[ -d "$app_dir/ten_packages/extension/speechmatics_asr_python" ]]; then
...
install_python_requirements() {
  ...
  if [[ -d "ten_packages/extension" ]]; then
    for extension in ten_packages/extension/speechmatics_asr_python; do
  ...
  if [[ -d "ten_packages/system" ]]; then
    for extension in ten_packages/system/speechmatics_asr_python; do
      if [[ -f "$extension/requirements.txt" ]]; then
        ${PIP_INSTALL_CMD} -r $extension/requirements.txt
      fi
    done
