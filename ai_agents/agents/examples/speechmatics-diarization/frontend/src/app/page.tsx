...
export default function HomePage() {
  ...
  const [language, setLanguage] = useState<string>("en")  // 语言选择状态，默认英文

  // 音量监控状态
  const [volumeLevels, setVolumeLevels] = useState<Map<number, number>>(new Map())
  const [showVolumeWarning, setShowVolumeWarning] = useState<boolean>(false)
  ...
  const speakerCards = useMemo<SpeakerCard[]>(() => {
    ...
    // 动态使用识别到的说话人，而不是硬编码的 KNOWN_SPEAKERS
    const allSpeakers = recognisedSpeakers.length > 0
      ? recognisedSpeakers
      : KNOWN_SPEAKERS

    const base: SpeakerCard[] = allSpeakers.map((name) => {
      ...
      if (transcriptLines.length === 0) {
        if (fallbackDetail) {
          ...
          transcriptLines.push("Waiting to speak.")
        }
      }
...
@keyframes spotlight-slide-in {
    ...
    (stream: ArrayBuffer) => {
      try {
        ...
        if (cache[message_id].length === total_parts) {
          ...
          const { text, is_final, text_ts, role, speaker } = payload

          // 调试日志：打印接收到的 payload
          console.log("[UI] 收到消息:", { text, is_final, text_ts, role, speaker })

          if (text && String(text).trim().length > 0) {
            // Add speaker label for user role (S1, S2, or NONE if empty)
            let displayText = text
            const speakerLabel = speaker || "NONE"  // 默认为 NONE

            if (role === "user") {
              displayText = `[${speakerLabel}] ${text}`
              console.log("[UI] 用户消息显示:", displayText)
            }
    ...
    try {
      ...
      client.on("volume-indicator", (volumes) => {
        ...
        volumes.forEach((volume) => {
          ...
          levels.set(uid as number, level)

          // 检查低音量用户
          if (level < 10 && uid !== userId) {
            hasLowVolume = true
        ...
        setShowVolumeWarning(hasLowVolume)
      })
      ...
      const micTrack = await AgoraRTC.createMicrophoneAudioTrack({
        ...
        encoderConfig: {
          sampleRate: 16000,      // 16kHz 采样率，匹配 Speechmatics 要求
          stereo: false,          // 单声道
          bitrate: 48,            // 48 kbps 比特率
        },
        // 启用音频处理功能
        AEC: true,                // 回声消除
        ANS: true,                // 背景噪音抑制
      ...
      // 设置麦克风音量（可选，根据需要调整）
      // micTrack.setVolume(100)  // 0-100，默认为100
  ...
  }, [channel, userId, join, joined, pending, language])
  ...
  const handleLanguageChange = useCallback(async (newLanguage: string) => {
    ...
    setPending(true)
    try {
      console.log(`[UI] 切换语言: ${language || "默认"} → ${newLanguage || "默认"}`)

      const { ok, msg } = await apiChangeLanguage({
        channel,
        language: newLanguage,
      })

      if (!ok) {
        throw new Error(msg || "Failed to change language")
      }

      // 更新本地语言状态
      setLanguage(newLanguage)
      console.log(`[UI] ✅ 语言切换成功: ${newLanguage || "默认"}`)
    } catch (err: any) {
      console.error("[UI] 语言切换失败", err)
      setError(err?.message || "Failed to change language")
    } finally {
      setPending(false)
    }
  }, [channel, joined, pending, language])
  ...
  const stop = useCallback(async () => {
    ...
    try {
      ...
      if (clientRef.current) {
        try {
          // 移除音量监听器
          clientRef.current.removeAllListeners("volume-indicator")
        } catch { }
        try {
          ...
          >
            ...
            >
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                ...
                >
                    <option value="cmn">普通话 (cmn)</option>
                    <option value="zh-CN">简体中文 (zh-CN)</option>
                    <option value="zh">中文 (zh)</option>
                    <option value="en">英语 (en)</option>
                    <option value="yue">粤语/广东话 (yue)</option>
                    ...
                    <option value="tl">菲律宾语 (tl)</option>
                ...
                {/* 音量监控指示器 */}
                {joined && volumeLevels.size > 0 && (
                  <div style={{
                    padding: "8px 12px",
                    borderRadius: 8,
                    border: `1px solid ${showVolumeWarning ? "#ff6b6b" : PANEL_BORDER_COLOR}`,
                    background: showVolumeWarning ? "#fff5f5" : "#ffffff",
                    display: "flex",
                    alignItems: "center",
                    gap: 8,
                  }}>
                    <span style={{ fontSize: 13, fontWeight: 500, color: TEXT_MUTED }}>
                      音量监控:
                    </span>
                    <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                      {Array.from(volumeLevels.entries()).map(([uid, level]) => (
                        <div
                          key={uid}
                          style={{
                            display: "flex",
                            ...
                            padding: "4px 8px",
                            borderRadius: 4,
                            background: level < 10 ? "#ffe0e0" : "#f0f0f0",
                            fontSize: 12,
                          }}
                          title={`用户 ${uid}: 音量 ${level}`}
                        >
                          ...
                          >
                            <div
                              style={{
                                ...
                                height: "100%",
                                background: level < 10 ? "#ff6b6b" : level < 30 ? "#ffd93d" : "#6bcf7f",
                                transition: "width 0.3s ease",
                              }}
                            />
                          </div>
                          <span style={{ fontSize: 11, color: level < 10 ? "#ff6b6b" : "#666" }}>
                            {level}
                          </span>
                        </div>
                      ))}
                    ...
                    {showVolumeWarning && (
                      <span style={{ fontSize: 11, color: "#ff6b6b", marginLeft: 4 }}>
                        ⚠️ 部分用户音量过低
                      </span>
                    )}
                  </div>
