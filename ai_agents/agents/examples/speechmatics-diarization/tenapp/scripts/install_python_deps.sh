...
PIP_INSTALL_CMD=${PIP_INSTALL_CMD:-"pip install"}
  ...
  if [[ -d "$app_dir/ten_packages/extension" ]]; then
    ...
    for extension in "$app_dir/ten_packages/extension"/*; do
      if [[ -d "$extension" && -f "$extension/requirements.txt" ]]; then
        ...
        ${PIP_INSTALL_CMD} -i https://mirrors.aliyun.com/pypi/simple/ --trusted-host mirrors.aliyun.com -r "$extension/requirements.txt"
      fi
    done
  else
...
main() {
  ...
  chmod +x "$APP_HOME/scripts/start.sh"
  if [[ -f "$APP_HOME/scripts/install_python_deps.sh" ]]; then
    chmod +x "$APP_HOME/scripts/install_python_deps.sh"
  fi
